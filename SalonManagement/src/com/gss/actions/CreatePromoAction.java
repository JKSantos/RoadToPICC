	...
	public String execute(){
		...
		if(!strNonExp.equalsIgnoreCase("on")){
			...
			if(strFree.equals("on")){
				...
				Promo promo = new Promo(1, strPromoName, strPromoDesc, strPromoGuidelines, 0, 1, serviceList, productList, packageList, "NON-EXPIRY", 1, PackageHelper.convertToSingleInt(type));
				promo.setRequirements(requirements);
				this.result = promoService.createPromo(promo);
			}
			else{
				
				Promo promo = new Promo(1, strPromoName, strPromoDesc, strPromoGuidelines, dblPromoPrice, 1, serviceList, productList, packageList, "NON-EXPIRY", 1, PackageHelper.convertToSingleInt(type));
				promo.setRequirements(requirements);
				this.result = promoService.createPromo(promo);
			}
