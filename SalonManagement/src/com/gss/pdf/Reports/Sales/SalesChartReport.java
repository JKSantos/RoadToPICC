package com.gss.pdf.Reports.Sales;
...
public class SalesChartReport {
	...
	private String destination = "/Sales_" + NumberGenerator.localDateTime() + ".pdf";
	...
	public String generateReport(SalesReport report) throws BadElementException, MalformedURLException, DocumentException, IOException{
		...
		String title = report.getType().toLowerCase() + " SALES REPORT AS OF YEAR " + cal.get(Calendar.YEAR);
	...
	public Document createDocument(){
        ...
        try {
			this.writer = PdfWriter.getInstance(document, new FileOutputStream(((ServletContext) ActionContext.getContext().get(StrutsStatics.SERVLET_CONTEXT)) 
	                .getRealPath("WEB-INF/Reports/Sales")
	                +
	                this.destination));
